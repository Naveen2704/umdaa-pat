...
export class SeComponent implements OnInit {
  ...
  disabled: boolean = false;
    ...
    constructor(private _service:PatientserviceService,private users: UsersService,private se:SeService,
   ...
   //text boxes
    ...
    mainTwo(id,title,e)
    {
      ...
      {
        this.main_section.push({section_id:id,section_description:e.target.value,section_image:''});
        console.log(this.main_section);
      }
      else if(e.target.value == '')
      {
    ...
    final(removeformId)
    {
      ...
      console.log(this.id);
       ...
       var getInfo1 = localStorage.getItem('SE'+this.id+'1');
       var getInfo2 = localStorage.getItem('SE'+this.id+'2');
      ...
      {
        this.impImageName = this.id+this.impIds[j].id;
        var getInfo3 = localStorage.getItem('SE'+this.id+this.impIds[j].id);
        if(getInfo3 != null)
        {
        ...
        this.se.saveData(this.id,removeformId,this.array123,this.main_section,this.getPreviewOne).subscribe((data)=>{
          ...
          if(data.code == 200)
          {
              this.se.getList(this.id).subscribe((data)=>{
                ...
                this.router.navigate(['/SeList/'+this.id]);
              });
              localStorage.removeItem('SE'+this.id+'1');
              localStorage.removeItem('SE'+this.id+'2');
              for(var j=0;j<this.impIds.length;j++)
              {
                localStorage.removeItem('SE'+this.id+this.impIds[j].id);
              }
          }
        });
       }
    ...
    openDialog(){
      this.router.navigate(['/secondsketch/'+'SE/'+this.id+'/'+this.id1]);
    ...
    form_images(a,b)
    {
      ...
      this.router.navigate(['/preview/'+'SE/'+this.id+'/'+b,a,this.id1]);
    }

    checkImage(a,b)
    {
      this.router.navigate(['/special/'+'SE/'+this.id+'/'+b+'/'+a]);
