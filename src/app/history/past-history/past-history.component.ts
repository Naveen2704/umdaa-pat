...
import { PastHistoryService } from '../past-history.service';
...
export class PastHistoryComponent implements OnInit {
  ...
  status: any;
    ...
    constructor(private users: UsersService,private se:PastHistoryService, 
      private route: ActivatedRoute, public dialog: MatDialog,
      private router: Router,
      private _notification:NotificationService,private _service: PatientserviceService,) { }
   ...
   //text boxes
    ...
    final(removeformId)
    {
      ...
      console.log(this.id);
       ...
       var getInfo1 = localStorage.getItem('Past'+this.id+'1');
       var getInfo2 = localStorage.getItem('Past'+this.id+'2');
      ...
      {
        var getInfo3 = localStorage.getItem('Past'+this.id+this.impIds[j].id);
        if(getInfo3 != null)
        {
        ...
        this.se.saveData(this.id,removeformId,this.array123,this.main_section,this.getPreviewOne).subscribe((data)=>{
          ...
          if(data.code == 200)
          {
              this.se.getList(this.id).subscribe((data)=>{
                ...
                this.router.navigate(['/PastHistoryList/'+this.id]);
              });
              localStorage.removeItem('Past'+this.id+'1');
              localStorage.removeItem('Past'+this.id+'2');
              for(var j=0;j<this.impIds.length;j++)
              {
                localStorage.removeItem('Past'+this.id+this.impIds[j].id);
              }
          }
        });
       }
    ...
    form_images(a,b)
    {
      ...
      this.router.navigate(['/preview/'+'Past/'+this.id+'/'+b,a,,this.id1]);
    }
  
  
    openDialog(){
      this.router.navigate(['/secondsketch/'+'Past/'+this.id+'/'+this.id1]);
    ...
    checkImage(a,b)
    {
      this.router.navigate(['/special/'+'Past/'+this.id+'/'+b+'/'+a]);
