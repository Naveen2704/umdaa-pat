...
export class PersonalHistoryComponent implements OnInit {
  ...
  disabled: boolean;
    ...
    constructor(private _service: PatientserviceService,private users: UsersService,private se:PersonalHistoryService, public dialog: MatDialog,private route: ActivatedRoute,private router: Router,private _notification:NotificationService) { }
    ...
    removeArrayData(a,b)
    {
      ...
      // this._notification.warning('Changes are lost');
   ...
   //text boxes
    ...
    final(removeformId)
    {
      ...
      console.log(this.id);
       ...
       var getInfo1 = localStorage.getItem('Personal'+this.id+'1');
       var getInfo2 = localStorage.getItem('Personal'+this.id+'2');
      ...
      {
        var getInfo3 = localStorage.getItem('Personal'+this.id+this.impIds[j].id);
        if(getInfo3 != null)
        {
        ...
        this.se.saveData(this.id,removeformId,this.array123,this.main_section,this.getPreviewOne).subscribe((data)=>{
          ...
          if(data.code == 200)
          {
              this.se.getList(this.id).subscribe((data)=>{
                ...
                this.router.navigate(['/PersonalHistoryList/'+this.id]);
              });
              localStorage.removeItem('Personal'+this.id+'1');
              localStorage.removeItem('Personal'+this.id+'2');
              for(var j=0;j<this.impIds.length;j++)
              {
                localStorage.removeItem('Personal'+this.id+this.impIds[j].id);
              }
          }
        });
       }
       else{
        // this._notification.error('No Data Selected');
    ...
    form_images(a,b)
    {
      ...
      this.router.navigate(['/preview/'+'Personal/'+this.id+'/'+b,a,,this.id1]);
    }
  
    openDialog(){
      this.router.navigate(['/secondsketch/'+'Personal/'+this.id+'/'+this.id1]);
     
    }

    checkImage(a,b)
    {
      this.router.navigate(['/special/'+'Personal/'+this.id+'/'+b+'/'+a]);
