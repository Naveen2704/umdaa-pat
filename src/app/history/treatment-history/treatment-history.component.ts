...
export class TreatmentHistoryComponent implements OnInit {
  ...
  disabled: boolean = false;
    ...
    constructor(private _service: PatientserviceService,private users: UsersService, public dialog: MatDialog,private se:TreatmentHistoryService, private route: ActivatedRoute,private router: Router,private _notification:NotificationService) { }
   ...
   //text boxes
    ...
    final(removeformId)
    {
      ...
      console.log(this.id);
       ...
       var getInfo1 = localStorage.getItem('Treatment'+this.id+'1');
       var getInfo2 = localStorage.getItem('Treatment'+this.id+'2');
      ...
      {
        var getInfo3 = localStorage.getItem('Treatment'+this.id+this.impIds[j].id);
        if(getInfo3 != null)
        {
        ...
        this.se.saveData(this.id,removeformId,this.array123,this.main_section,this.getPreviewOne).subscribe((data)=>{
          ...
          if(data.code == 200)
          {
              this.se.getList(this.id).subscribe((data)=>{
                ...
                this.router.navigate(['/TreatmentHistoryList/'+this.id]);
              });
              localStorage.removeItem('Treatment'+this.id+'1');
              localStorage.removeItem('Treatment'+this.id+'2');
              for(var j=0;j<this.impIds.length;j++)
              {
                localStorage.removeItem('Treatment'+this.id+this.impIds[j].id);
              }
          }
        });
       }
    ...
    form_images(a,b)
    {
      ...
      this.router.navigate(['/preview/'+'Treatment/'+this.id+'/'+b,a,,this.id1]);
    }
  
    checkImage(a,b)
    {
      this.router.navigate(['/special/'+'Treatment/'+this.id+'/'+b+'/'+a]);
  
    }

  
    openDialog(){
      this.router.navigate(['/secondsketch/'+'Treatment/'+this.id+'/'+this.id1]);
