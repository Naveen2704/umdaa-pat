...
export class SocialHistoryComponent implements OnInit {
  ...
  disabled: boolean = false;
    ...
    constructor(private _service: PatientserviceService,private users: UsersService,  public dialog: MatDialog, private se:SocialHistoryService, private route: ActivatedRoute,private router: Router,private _notification:NotificationService) { }
   ...
   //text boxes
    ...
    final(removeformId)
    {
      ...
      console.log(this.id);
       ...
       var getInfo1 = localStorage.getItem('Social'+this.id+'1');
       var getInfo2 = localStorage.getItem('Social'+this.id+'2');
      ...
      {
        var getInfo3 = localStorage.getItem('Social'+this.id+this.impIds[j].id);
        if(getInfo3 != null)
        {
        ...
        this.se.saveData(this.id,removeformId,this.array123,this.main_section,this.getPreviewOne).subscribe((data)=>{
          ...
          if(data.code == 200)
          {
              this.se.getList(this.id).subscribe((data)=>{
                ...
                this.router.navigate(['/SocialHistoryList/'+this.id]);
              });
              localStorage.removeItem('Social'+this.id+'1');
              localStorage.removeItem('Social'+this.id+'2');
              for(var j=0;j<this.impIds.length;j++)
              {
                localStorage.removeItem('Social'+this.id+this.impIds[j].id);
              }
          }
        });
       }
    ...
    form_images(a,b)
    {
      ...
      this.router.navigate(['/preview/'+'Social/'+this.id+'/'+b,a,,this.id1]);
    }
  
    openDialog(){
      this.router.navigate(['/secondsketch/'+'Social/'+this.id+'/'+this.id1]);
    ...
    checkImage(a,b)
    {
      this.router.navigate(['/special/'+'Social/'+this.id+'/'+b+'/'+a]);
