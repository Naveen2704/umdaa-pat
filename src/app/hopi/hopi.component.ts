...
import { HopiService } from './hopi.service';
...
@Component({
  ...
  styleUrls: ['./hopi.component.css']
    ...
    export class HopiComponent implements OnInit {
      // export class FinalSeComponent implements OnInit {
        ...
        getPreviewOne: any =[];
        impIds: any=[];
        hopiSketch: boolean = false;
  obj: any;
  status: any;
  disabled: boolean = false;
      
        constructor(private service: PatientserviceService,private se:HopiService, private route: ActivatedRoute,private router: Router,public _notification:NotificationService) 
        { }
       ...
       //text boxes
        ...
        final(removeformId)
        {
          ...
          console.log(this.id);
           ...
           var getInfo1 = localStorage.getItem('HOPI,'+this.removeformId+',one1');
           var getInfo2 = localStorage.getItem('HOPI,'+this.removeformId+',one2');
          ...
          {
            var getInfo3 = localStorage.getItem('HOPISpecialImage,'+this.impIds[j].id);
            if(getInfo3 != null)
            {
            ...
            this.se.saveData(this.id,removeformId,this.array123,this.main_section,this.getPreviewOne).subscribe((data)=>{
              ...
              if(data.code == 200)
              {
                  this.se.getList(this.id).subscribe((data)=>{
                    ...
                    this.router.navigate(['/HopiList/'+this.id]);
                  });
              }
            });
           }
        ...
        open(a)
        {
          this.router.navigate(['/touch/'+a,this.removeformId]);
        }
        ...
        form_images(a,b)
        {
          ...
          this.router.navigate(['/preview/'+'HOPI',b,a]);
        }
